...
public class Server implements Runnable {
	...
	private void printHelp() {
		...
		System.out.println("/kick [user ID or user name] - kicks a user.");
	...
	private void sendStatus() {
		...
		for(int i = 0; i < clients.size() - 1; i++) {
			users += clients.get(i).name.trim() + "/n/";
		}
		users += clients.get(clients.size() -1).name.trim() + "/e/";
		sendToAll(users);
	}
